...
window.UIContextMenu = (function (utils) {
    ...
    /**
     * Открывает контекстное меню в экранной позиции.
     * Конвертирует экранные координаты в мировые для setPoint.
     * Позиционирует меню так, чтобы оно не выходило за пределы map-wrap.
     *
     * @param {number} sx — экранная X координата (пиксели canvas)
     * @param {number} sy — экранная Y координата (пиксели canvas)
     */
    function openMenuAt(sx, sy) {
        if (!menu) return;
        const view = deps.getView();

        /** Конвертируем экранные координаты в мировые (метры) */
