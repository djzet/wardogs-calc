window.MapTiles = (function () {
    ...
    function getTile(z, x, y, tilesConfig, onLoaded) {
        ...
        const img = new Image();
        t = { img, loaded: false, error: false, loading: true };
        ...
        if (tilesConfig.usePMTiles && window.PMTilesLoader && window.PMTilesLoader.isInitialized(mapId)) {
            window.PMTilesLoader.getTileUrl(mapId, z, x, y)
                .then(url => {
                    if (url) {
                        img.src = url;
                    } else {
                ...
                .catch(err => {
                    console.error(`[PMTiles] Tile error ${z}/${x}/${y}:`, err);
                    t.error = true;
                    t.loading = false;
                });
        } else {
