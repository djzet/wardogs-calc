# WARDOGS — Калькулятор міномета та артилерії

[Русский](README.md) · [English](README.en.md) · [Deutsch](README.de.md) · [Français](README.fr.md) · [Español](README.es.md) · [Polski](README.pl.md) · [Українська](README.uk.md) · [Türkçe](README.tr.md) · [中文](README.zh.md)

Неофіційний інтерактивний калькулятор міномета та артилерії для тактичного
шутера **WARDOGS**. Дистанція, азимут, кут підвищення (mils), час польоту —
у два кліки по карті 16×16 км.

**Live Demo:** https://djzet.github.io/wardogs-calc/

| Desktop | Mobile |
| :-: | :-: |
| ![desktop](docs/screenshots/desktop.png) | ![mobile](docs/screenshots/mobile.png) |

## Можливості

- Інтерактивна карта з тайловим зумом і панорамуванням
- Дистанція, азимут, кут підвищення (mils), час польоту
- Міномет (700 м) і артилерія (2,5 км)
- Share-посилання, автозбереження, теми
- 9 мов, мобільна версія

Повний список — у [docs/features.md](docs/features.md).

## Документація (RU)

- [development.md](docs/development.md) — запуск та архітектура
- [features.md](docs/features.md) — функції та дані зброї
- [localization.md](docs/localization.md) — переклади
- [contributing.md](docs/contributing.md) — як долучитися

## Контакти

- egor.silaev2003@yandex.ru
- [Wardogs СНД](https://discord.gg/kwxrTCJxre)
- @djzet

## Ліцензія

MIT © Egor Silaev